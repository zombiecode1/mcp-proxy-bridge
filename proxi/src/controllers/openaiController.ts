...
export const handleChatCompletion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (isStream) {
      ...
      // Best-effort: some models emit <think> blocks; strip them to keep responses clean for OpenAI-compatible clients.
      const sanitized: any = { ...(completion as any) };
      if (Array.isArray(sanitized.choices)) {
        for (const c of sanitized.choices) {
          const content = c?.message?.content;
          if (typeof content === 'string') {
            c.message.content = stripThinkBlocks(content);
          }
        }
      }
...
export const handleDashboard = async (req: Request, res: Response) => {
  ...
  const html = [
    ...
    '<div style="font-size:.9rem;color:#8b949e;margin-top:6px">Powered by <strong>' + identityName + '</strong>' + (identityOwner ? ' — ' + identityOwner : '') + (identityWebsite ? ' <a href="' + identityWebsite + '" style="color:#58a6ff;text-decoration:none;margin-left:6px" target="_blank">' + identityWebsite + '</a>' : '') + '</div>',
