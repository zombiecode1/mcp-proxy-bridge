...
export class MawlanaRouter {
  ...
  async route(
    ...
    if (prefKey && (ROUTE_PREFERENCES as any)[prefKey]) {
      const model = this.pickFirstAvailable(ROUTE_PREFERENCES[prefKey].models) || 'llama-3.3-70b-versatile';
      return {
        model,
        category: prefKey,
    ...
    if (lastMsg.length < 15) {
      const model = this.pickFirstAvailable(ROUTE_PREFERENCES.chat.models) || 'llama-3.3-70b-versatile';
    ...
    const model = this.pickFirstAvailable(pref.models) || 'llama-3.3-70b-versatile';
  ...
  private async classifyWithSmallModel(input: string, categories: string[]): Promise<RouteKey> {
    const classifierModel =
      this.pickFirstAvailable(['llama-3.1-8b-instant', 'groq/compound-mini', 'allam-2-7b']) ||
  ...
  getAllRoutes(): Record<string, RouteDecision> {
    ...
    (Object.keys(ROUTE_PREFERENCES) as RouteKey[]).forEach((k) => {
      const model = this.pickFirstAvailable(ROUTE_PREFERENCES[k].models) || 'llama-3.3-70b-versatile';
      out[k] = {
        model,
        category: k,
